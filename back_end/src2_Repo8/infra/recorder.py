...
def record_event(request_id: str, node_name: str,status, input_data, output_data):
    """记录事件"""
    conn.execute(
        " INSERT INTO agent_event (request_id, node, status, input_data, output_data) VALUES (?, ?, ?, ?, ?)",
        (
            request_id,
            node_name,
            status,
